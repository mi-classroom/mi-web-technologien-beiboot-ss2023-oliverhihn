...
export async function shutdownScene() {
...
function placeObject() {
  if (reticle.visible && modelLoaded) {
    ...
    window.eventBus.$emit("modelAddedToScene", {
      id: modelLoaded.userData.productId,
    });
    renewModel(
      modelLoaded.userData.productId,
      modelLoaded.userData.modelFile,
      modelLoaded.scale.x
    );
  }
}
...
function deleteTargetObject() {
  ...
  window.eventBus.$emit("modelDeletedFromScene", {
    id: targetObject.userData.productId,
  });
  targetObject = null;
}
